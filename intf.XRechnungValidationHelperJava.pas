...
begin
  ...
  try
    ...
    if _TrueIfUBL_FalseIfCII then
      cmd.Add(QuoteIfContainsSpace(JavaRuntimeEnvironmentPath+'bin\java.exe')+' -jar '+
             QuoteIfContainsSpace(ValidatorLibPath+'libs\Saxon-HE-9.9.1-7.jar')+' -s:'+QuoteIfContainsSpace(_InvoiceXMLFilename)+
             ' -xsl:'+QuoteIfContainsSpace(VisualizationLibPath+'xsl\ubl-invoice-xr.xsl')+
      ...
      cmd.Add(QuoteIfContainsSpace(JavaRuntimeEnvironmentPath+'bin\java.exe')+' -jar '+
             ...
             ' -o:'+QuoteIfContainsSpace(ChangeFileExt(_InvoiceXMLFilename,'-xr.xml')));
    cmd.Add(QuoteIfContainsSpace(JavaRuntimeEnvironmentPath+'bin\java.exe')+' -jar '+
             QuoteIfContainsSpace(ValidatorLibPath+'libs\Saxon-HE-9.9.1-7.jar')+' -s:'+QuoteIfContainsSpace(ChangeFileExt(_InvoiceXMLFilename,'-xr.xml'))+
             ' -xsl:'+QuoteIfContainsSpace(VisualizationLibPath+'xsl\xrechnung-html.xsl')+
